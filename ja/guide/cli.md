...
3. ソースのオプションの `probe()` メソッドを実行します。HTTP/HTTPS ソースの場合は HTTP HEAD リクエスト、BigQuery の場合は認証情報の確認です。
...
| `--kid <name>` | 自動生成 (UUID プレフィックス) | 鍵の短い識別子。ログ、認証済みの `/health/details` と `/models` エンドポイント、ローテーション手順で使用されます。 |
