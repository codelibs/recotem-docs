---
title: アーキテクチャ
description: "Recotem のアーキテクチャ解説。レシピ駆動のレコメンダー学習と配信を署名付きアーティファクトで分離し、ホットスワップで無停止更新する仕組みを説明します。"
---
...
| アーティファクトファイル | 署名済みバイナリ。`kid_bytes \|\| header_json \|\| payload` の内部を改ざんすると HMAC 検証が失敗する。4 バイトの `header_len` フィールドはこの範囲外で、これを動かしても `verify_hmac` は通り、1 層下のヘッダー JSON パースまたはデシリアライザで終了コード 5 として検出される。分割位置をずらせるだけで、バイトを注入することはできない。 | HMAC で認証済み |
