...
検証に失敗した場合、終了コードで何が問題かがわかります。レシピスキーマ・環境変数・パススキームのエラーは **2**、データソースエラーは **3**、未知のアルゴリズム名は **4**、開けない `training.storage_path` (サポートされないダイアレクト、またはエクストラ未インストールのドライバ) は **8** です。[終了コード](/2.2/ja/docs/exit-codes)を参照してください。
...
典型的な原因を含む完全な終了コードリファレンスについては [終了コード](/2.2/ja/docs/exit-codes) を参照してください。
