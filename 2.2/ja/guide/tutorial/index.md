...
シードにはステップ 4 のレスポンスに含まれる任意の `item_id` を使えます。レスポンスの形式は `:recommend` と同じで、シードアイテム自身が結果に現れることはありません。全フィールドについては[サービング API](/2.2/ja/docs/serving-api#post-v1-recipes-name-recommend-related) を参照してください。
...
- [レシピの基本](/2.2/ja/guide/recipe-basics) — レシピの各セクションを詳しく理解する
- [CLI リファレンス](/2.2/ja/guide/cli) — `train`、`serve` などのコマンドの全フラグ
- [レシピリファレンス](/2.2/ja/docs/recipe-reference) — すべてのレシピフィールドの詳細ドキュメント
- [バッチとスケジューリング](/2.2/ja/guide/batch) — cron スケジュールで学習を実行する
