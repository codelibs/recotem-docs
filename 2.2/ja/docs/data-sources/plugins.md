...
プラグインのオーサリング契約の全詳細 (クラスの形状、`FetchContext`、テスト、パッケージング、互換性) については [Plugin Authoring](/2.2/ja/docs/plugin-authoring) を参照してください。
...
上記の `echo` プラグインは `examples/plugins/echo-source/` にある正規のリファレンス実装です。合成 DataFrame を生成し、実データなしで学習〜配信の全フローを実行する必要がある CI パイプラインに役立ちます。 `timestamp` 列はエポック整数を保持するため `schema.time_unit` が必須であり、また行数は評価がちょうどゼロスコアにならないように設定されています — どちらも [Plugin Authoring](/2.2/ja/docs/plugin-authoring) で説明しています。
