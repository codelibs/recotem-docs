...
The image runs as a non-root user (UID 1000). Mount your recipe directory and artifact directory as volumes when training or serving. The tutorial's `compose.yaml` shows a complete working example — see the [Tutorial](/2.2/guide/tutorial/).
...
Both `RECOTEM_SIGNING_KEYS` and `RECOTEM_API_KEYS` accept multiple comma-separated entries (`kid1:value,kid2:value`) to enable key rotation without downtime. See the [Operations](/2.2/docs/operations) guide for the rotation procedure.

## Next step

Follow the [Tutorial](/2.2/guide/tutorial/) to train and serve your first recommender in under 10 minutes.
