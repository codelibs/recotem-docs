...
| `sha256` | string | optional (required when `path` is `http://` or `https://`) | 64-char lowercase hex; verified against the fetched bytes; mismatch raises `DataSourceError`. Checked by `recotem validate` and at model load, and **not** by `recotem train` — see [Where each `sha256` pin is checked](#where-each-sha256-pin-is-checked). |
...
| Field | Type | Default | Notes |
|-------|------|---------|-------|
| `algorithms` | list[string] | required | `IALS`, `CosineKNN` (alias `CosinekNN`), `TopPop`, `RP3beta`, `DenseSLIM`, `TruncatedSVD`, and `BPRFM` (**requires the `bprfm` extra** — without it `validate` and `train` both exit 4 with `irspack does not know recommender class 'BPRFMRecommender'`, before any data is fetched; see [Installation](/2.2/guide/installation#optional-extras)). **A BPRFM recipe cannot answer `:recommend-related` or `:batch-recommend-related`** — it is the only supported algorithm without `get_score_cold_user`, so those two verbs return `501 RELATED_NOT_SUPPORTED`; see [Serving API](/2.2/docs/serving-api#post-v1-recipes-name-recommend-related). Full irspack class names (e.g. `IALSRecommender`) are also accepted, and names are matched case-insensitively — so `[IALS, ials]` names one algorithm twice. Duplicates collapse to their first occurrence and a `duplicate_algorithms_collapsed` warning is logged, keeping the whole `n_trials` budget on the distinct algorithms. Hyperparameter ranges come from each recommender's `default_suggest_parameter` in irspack — they are not user-tunable from the recipe. |
