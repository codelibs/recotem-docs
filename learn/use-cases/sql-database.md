...
Set `sslmode=require` (or stricter: `verify-ca`, `verify-full`) on PostgreSQL, or `ssl_ca=/path/to/ca.pem` (or `ssl_verify_cert=true`) on MySQL/MariaDB. `?ssl=true` is **not** a usable spelling — PyMySQL's `ssl` parameter takes a mapping or an `ssl.SSLContext`, never a string, so any non-empty scalar value fails inside the driver before it connects. Recotem does not enforce TLS, but it emits a `sql_dsn_tls_not_configured` warning when the DSN looks plaintext.
:::
...
- `algorithms` lists three implicit-feedback learners; Recotem runs an Optuna search across them and keeps the best by `ndcg@20`. `TopPop` is a cheap popularity candidate, but note what it is **not**: it is another entry in the search, not a quality gate. If it scores highest on recotem's internal split it is simply what ships, and **beating popularity is not a meaningful bar** — see [Is the model any good?](/learn/basics/collaborative-filtering#is-the-model-any-good). Other options include `CosineKNN`, `DenseSLIM`, and `TruncatedSVD`; `BPRFM` needs `lightfm` installed alongside Recotem and cannot answer the related verbs, so check [Installation](/guide/installation#optional-extras) before listing it.
