...
Use any `item_id` from the Step 4 response as the seed. The response has the same shape as `:recommend`, and the seed items themselves never appear in it. See [Serving API](/docs/serving-api#post-v1-recipes-name-recommend-related) for the full field list.
