...
2.1.0 retracts the annotation once it can show it no longer holds, and logs `artifact_load_error_cleared` (INFO) when it does; see [Watcher and registry semantics](/docs/operations#watcher-and-registry-semantics).
...
That matters because `sql-sqlite` is the example the [SQL data-source documentation](/docs/data-sources/sql) points at. If you started from it, you have a recipe whose `best_score` moves on the first retrain after upgrading, for a reason nothing in the recipe explains. Grep your own recipes for the same combination.
...
The `source.sha256` row is the one to check your retry logic against. [Kubernetes Deployment](/docs/deployment/kubernetes) classifies exit 7 as *retry* (a transient network fault) and exit 3 as a data-source error. A permanently wrong `sha256` pin on a local path was reported as retryable under 2.0.0, so a CronJob would retry bytes that can never match; 2.1.0 reports 3.

A malformed `RECOTEM_SIGNING_KEYS` also moved, from 5 to 8 — see [Unchanged by this upgrade](#unchanged-by-this-upgrade) below, which covers it in the context of the artifact format it does *not* affect. The full per-code reference is in [Exit Codes & Errors](/docs/exit-codes).
...
The full runbook, including the zero-downtime caveat that this upgrade breaks, is [irspack version skew](/docs/operations#irspack-version-skew).
...
The mechanism, the verified-pair table, the fail-open and fail-closed cases, and the `RECOTEM_ALLOW_IRSPACK_VERSION_SKEW` escape hatch are all documented once, in [irspack version skew](/docs/operations#irspack-version-skew). Two facts from that rule are specific to planning an upgrade:
...
See the [Tutorial](/guide/tutorial/) for the full walkthrough.
