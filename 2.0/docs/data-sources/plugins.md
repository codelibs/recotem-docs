...
For the full plugin authoring contract (class shape, `FetchContext`, testing, packaging, compatibility), see [Plugin Authoring](/2.0/docs/plugin-authoring).
...
| `Config` | `pydantic.BaseModel` subclass | yes | Describes the recipe sub-fields for this source. All fields appear under `source:` in the YAML alongside `type:`. The loader passes the entire `source:` mapping to `Config.model_validate(...)`. `Config` **must** declare the discriminator field itself as `type: Literal["<type_name>"] = "<type_name>"`, matching `type_name` exactly; a missing field, a field that is not a `typing.Literal`, or a `Literal` value that disagrees with `type_name` raises `DataSourceError` at plugin-discovery time. |
