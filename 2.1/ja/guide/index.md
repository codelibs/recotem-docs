...
この設定ファイルを **レシピ** と呼びます。レシピには「どのデータを使うか」「どのように学習するか」「どこで配信するか」が記述されています。1 つのレシピが 1 つのモデルを生み出し、それを HTTP エンドポイント群 (単発/バッチ、user-to-item / item-to-item) として公開します。
...
2. **`recotem serve`** はアーティファクトディレクトリを監視し、各モデルを `/v1/recipes/{name}:recommend` (および関連 verb エンドポイント) という REST エンドポイントとして読み込みます。`recotem train` が新しいアーティファクトを生成すると、サーバーが自動的に検出して読み込みます。再起動は不要です。
...
任意のスケジューラ    ローカル FS / S3 / GCS          POST /v1/recipes/{name}:recommend
...
- [インストール](/2.1/ja/guide/installation) — pip と Docker のセットアップ、鍵の生成
- [チュートリアル](/2.1/ja/guide/tutorial/) — 実際の購買ログデータセットを使ったエンドツーエンドの例
- [レシピの基本](/2.1/ja/guide/recipe-basics) — レシピファイルの各セクションを理解する
- [CLI リファレンス](/2.1/ja/guide/cli) — 全 6 コマンドとそのフラグ
