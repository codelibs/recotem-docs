...
### トライアルあたりの時間は 3 乗、メモリは 2 乗で増加し、どちらも `training.parallelism` と乗算される

irspack はサイドごとに密な `Fᵀ F` グラム行列を構成し、Cholesky 分解で解きます。2 つのコストは異なるスケールをするため、ホストをサイジングする際は分けて考える価値があります。**時間**はエンコード次元の **3 乗**で増加し (分解そのもの)、**メモリ**は **2 乗**でしか増加しません — グラム行列は float64 で `dim² × 8` バイトであり、これは下表のメモリ列によく追随します (式は 200 MB / 800 MB / 3.2 GB を与え、実測は 200 MB / 771 MB / 3 GB です。グラム行列が支配的ですが唯一の割り当てではありません)。irspack はどちらでもエラーを出さず、劣化するだけです。トライアルあたりの実測値:
...
| `BPRFMRecommender` | **検証不能** — irspack はこれを別途インストールする `lightfm` パッケージの背後にゲートしているが、`lightfm` には Python 3.12 対応リリースがないため irspack はこのクラスをエクスポートせず、Recotem は学習できない。テーブルにないことは*未証明*を意味し、既知の破損を意味しない。 |
