...
The allow-list is frozen per irspack 0.5.x. If irspack adds or renames recommender classes, the list and the CHANGELOG entry are updated together.
...
::: tip RECOTEM_RECIPE_GCP_PROJECT is allowed
...
- **Key compromise**: rotate immediately. The four-step procedure is in
  [operations guide — Signing key rotation](./operations#signing-key-rotation).
